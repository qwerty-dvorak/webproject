...
function createOscillator(
  ...
  gainNode.gain.linearRampToValueAtTime(actualVolume, audioContext.currentTime + attack);
...
function generateJumpSound(): void {
  ...
  const mainOsc = createOscillator('square', startFreq, 0.25, 0.15, 0.01, 0.2);
  
  // Secondary oscillator for richer sound
  const secondaryOsc = createOscillator('square', startFreq * 1.5, 0.2, 0.1, 0.01, 0.1);
  ...
  if (mainOsc && audioContext) {
    ...
    mainOscillator.frequency.exponentialRampToValueAtTime(endFreq, audioContext.currentTime + 0.15);
    
    const gainNode = audioContext.createGain();
    gainNode.gain.setValueAtTime(0, audioContext.currentTime);
    gainNode.gain.linearRampToValueAtTime(0.25 * sfxState.current.sfxVolume, audioContext.currentTime + 0.05);
  ...
  if (secondaryOsc && audioContext) {
    const secondaryOscillator = (secondaryOsc as any).context.createOscillator();
    secondaryOscillator.frequency.setValueAtTime(startFreq * 1.5, audioContext.currentTime);
    secondaryOscillator.frequency.exponentialRampToValueAtTime(endFreq * 1.5, audioContext.currentTime + 0.12);
    
    const gainNode = audioContext.createGain();
    gainNode.gain.setValueAtTime(0, audioContext.currentTime);
    gainNode.gain.linearRampToValueAtTime(0.1 * sfxState.current.sfxVolume, audioContext.currentTime + 0.03);
...
function generateGameStartSound(): void {
  ...
  sequence.forEach((freq, index) => {
    setTimeout(() => {
      ...
      gainNode.gain.linearRampToValueAtTime(0.2 * baseVolume, ctx.currentTime + 0.02);
      gainNode.gain.linearRampToValueAtTime(0.1 * baseVolume, ctx.currentTime + 0.08);
      gainNode.gain.linearRampToValueAtTime(0, ctx.currentTime + 0.1);
      
      // Volume envelope for harmony
      const harmonyGain = ctx.createGain();
      harmonyGain.gain.setValueAtTime(0, ctx.currentTime);
      harmonyGain.gain.linearRampToValueAtTime(0.08 * baseVolume, ctx.currentTime + 0.02);
...
function generateGameOverSound(): void {
    ...
    sequence.forEach((freq, index) => {
      setTimeout(() => {
        ...
        const gainNode = ctx.createGain();
        gainNode.gain.setValueAtTime(0, ctx.currentTime);
        gainNode.gain.linearRampToValueAtTime(0.2 * baseVolume, ctx.currentTime + 0.05);
        gainNode.gain.linearRampToValueAtTime(0, ctx.currentTime + 0.4);
        
        const minorGain = ctx.createGain();
        minorGain.gain.setValueAtTime(0, ctx.currentTime);
        minorGain.gain.linearRampToValueAtTime(0.1 * baseVolume, ctx.currentTime + 0.05);
...
function generateHitSound(): void {
  ...
  noiseGain.gain.linearRampToValueAtTime(0.3 * sfxState.current.sfxVolume, ctx.currentTime + 0.005);
  noiseGain.gain.exponentialRampToValueAtTime(0.001, ctx.currentTime + 0.1);
  
  const impactGain = ctx.createGain();
  impactGain.gain.setValueAtTime(0, ctx.currentTime);
  impactGain.gain.linearRampToValueAtTime(0.4 * sfxState.current.sfxVolume, ctx.currentTime + 0.005);
  ...
  crackGain.gain.linearRampToValueAtTime(0.2 * sfxState.current.sfxVolume, ctx.currentTime + 0.001);
...
function generateKeyTypeSound(): void {
  ...
  secondGain.gain.linearRampToValueAtTime(baseVolume * 0.5, ctx.currentTime + 0.003);
...
export const sfxState = {
  ...
  current: {
    sfxVolume: getStoredValue<number>('pixelTanks.sfxVolume', 0.7),
    isMuted: getStoredValue<boolean>('pixelTanks.sfxMuted', false)
  ...
  setSfxVolume: (volume: number) => {
    ...
    localStorage.setItem('pixelTanks.sfxVolume', JSON.stringify(volume));
  ...
  toggleMute: () => {
    ...
    localStorage.setItem('pixelTanks.sfxMuted', JSON.stringify(sfxState.current.isMuted));
    sfxState.notifyListeners();
  },
